...
class AttentionScorer:
    ...
    def __init__(
        ...
        #this calc how long the driver has been inattentive
    ...
    # ):
    def eval_scores(
        self, t_now, gaze_score, head_roll, head_pitch, head_yaw
    ):
        """
        ...
        #asleep if closure time > ear time threshold
        #looking away if not looking ahead time > threshold
        #distracted if roll, pitch, yaw > threshold for more than threshold time
        ...
        #print(f"Distracted Time: {self.distracted_time}")
        ...
        if self.verbose:  # print additional info if verbose is True
            print(
                f"looking away:{looking_away}\tdistracted:{distracted}"
            )

        return looking_away, distracted
