...
from structuredFaceRecog import load_database, save_to_database, recognize_face
...
def overallCali(frame, ear, lip_distance):
    ...
    global ear_values, blink_values, mar_values
    ...
    if not calibrated_ear:
        ...
        # print(len(ear_values),"ear values")
        # print(calibration_frames, "calibration frames")
        if len(ear_values) < calibration_frames:
            print(f"[INFO] EAR Calibration in progress... {len(ear_values)}/{calibration_frames}")
        elif len(ear_values) == calibration_frames:
            ...
            print(personal_eye_threshold, "personal_eye_threshold")
...
def drowsyDetection(ear, lip_distance, personal_eye_threshold, personal_blink_rate_threshold, personal_mar_threshold):
    global COUNTER, start_time, closure_duration, blink_start_time, close_du, yawns,blink_rate_per_minute, yawn_detected,blink, ALARM_ON
    ...
    if ear < personal_eye_threshold:
        ...
        if start_time is not None and time.time() - start_time >= 2.0:
            blink += 1
            closure_duration = time.time() - start_time  # Calculate duration of eye closure
            close_du = closure_duration
            start_time = None  # Reset timer after counting the blink
...
def save_driver_name(driver_name):
    return driver_name

def main():
    global calibrated_ear,calibrated_mar,calibrated_blink_rate, ear, lip_distance, message1, message2, message3, not_recognize, aggressive, lower_limit, higher_limit, ALARM_ON
    global personal_eye_threshold, personal_blink_rate_threshold, personal_mar_threshold
    global warning
    # Load the known faces and calibration data
    known_face_encodings, known_face_names, calibration_data = load_database()

    first_face_encoding = None  # Initialize
    ...
    not_recognize = True #To ensure the face recognition only run once
    ...
    while True:
        ...
        if rects:
            ...
            largest_face_encoding = face_recognition.face_encodings(rgb_frame, [(largest_face.top(), largest_face.right(), largest_face.bottom(), largest_face.left())])[0]
            ...
            # Face recognition and calibration
            name, face_location = recognize_face(frame, known_face_encodings, known_face_names)
            print(f"Face recognition completed")
            #print(name, face_location)

            earmarCalc(shape_np, frame)
            ear, lip_distance = earmarCalc(shape_np, frame)

            if not_recognize:
                #if the driver is known
                if name:
                    facename = name
                    # Load the driver's calibration data from the database
                    driver_calibration_data = calibration_data.get(name)
                    #first_face_encoding = known_face_encodings.get(name)
                    # Assuming `known_face_encodings` is a list of face encodings and `known_names` is a list of corresponding names
                    for encoding, current_name in zip(known_face_encodings, known_face_names):
                        if current_name == name:
                            first_face_encoding = encoding
                            break
                    else:
                        first_face_encoding = None  # Handle the case where no matching name is found
                    ...
                    if driver_calibration_data:
                        ...
                        #first_face_encoding = driver_face_encoding.get('encodings')
                    ...
                    calibrated_mar = False
                    if face_location:
                        print("Unknown face detected! Please go through calibration.")
                        facename = "Unknown"
                        while not driver_name:
                            driver_name = save_driver_name(driver_name)
                            # Start calibration phase (collect EAR, MAR, eye blink, etc.)
                            # driver_name = save_driver_name(name)
                    
                        while not (calibrated_ear and calibrated_blink_rate and calibrated_mar):
                            overallCali(frame, ear, lip_distance)
                            ear_thres, blink_thres, mar_thres = overallCali(frame, ear, lip_distance)
                        
                        calibration_data[driver_name] = {
                        ...
                        # Save new face encoding, name, and calibration data
                        first_face_encoding = face_recognition.face_encodings(frame, [face_location])[0]
                        known_face_encodings.append(first_face_encoding)  # You'll need to get the face encoding
                        known_face_names.append(driver_name)
                        save_to_database(known_face_encodings, known_face_names, calibration_data)
                        print(f"Calibration complete for {driver_name}.")
            ...
            if calibrated_ear and calibrated_blink_rate and calibrated_mar:
                if not(message3):
                    print('Start Drowsiness Detection')
                    message3 = True
                if first_face_encoding is not None:
                    distances = face_recognition.face_distance([largest_face_encoding], first_face_encoding)
                    best_match_index = np.argmin(distances) if len(distances) > 0 else -1
                    if best_match_index != -1 and distances[best_match_index] < 0.6:
                        print('same driver')
                    else:
                        print('Different driver detected!')
                        cv2.putText(
                        frame,
                        "DRIVER MISSING!",
                        (10, 300),
                    ...
                    print("First face encoding is None, skipping distance calculation.")
                ...
                if physical_state > 2:
                    if not ALARM_ON:
                        ...
                        if alarm_path:
                            ...
                            t = Thread(target=intensity_alarm, args=(drowsy_path, intensity))
                            t.daemon = True
                            t.start()
                else:
                    ALARM_ON = False
                ...
                def get_driver_state_label(driver_state_value):
                    if driver_state_value <= lower_limit:
                        return "Low"
                    elif lower_limit < driver_state_value <= higher_limit:
                        return "Medium"
                    else:
                        return "High"
                ...
                if driver_state_label == "Low" and aggressive:
                    lower_limit = max(0, lower_limit - 1)
                    higher_limit = max(0, higher_limit - 1)
                elif driver_state_label == "High" and not aggressive:
                    lower_limit = min(10, lower_limit + 1)
                    higher_limit = min(10, higher_limit + 1)                  

                # Display the linguistic term
                cv2.putText(frame, f"Drowsy Level: {driver_state_label}", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    ...
    return {"warning": warning, "physical_state": physical_state, "facename": facename}

if __name__ == "__main__":
    main()

#problem: if name there sometimes can run, sometimes cannot